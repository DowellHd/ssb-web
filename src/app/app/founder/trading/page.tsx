...
import { getOptionSnapshot } from '@/lib/api/market-data';
...
// ============================================================================
// Live P&L component (uses live option price from backend)
// ============================================================================
...
function LivePnL({
  ...
  const [snapshot, setSnapshot] = useState<OptionSnapshot | null>(null);
  const [loading, setLoading] = useState(false);

  const fetchSnapshot = useCallback(async () => {
    if (!intent.expiry || !intent.legs || intent.legs.length === 0) return;
    const leg = intent.legs[0] as { right: 'call' | 'put'; strike: string; qty: number };
    if (!leg.right || !leg.strike) return;
    setLoading(true);
    try {
      const snap = await getOptionSnapshot(
        intent.symbol,
        intent.expiry,
        leg.right,
        parseFloat(leg.strike),
      );
      setSnapshot(snap);
    } finally {
      setLoading(false);
  ...
  const leg = intent.legs?.[0] as { side: string; qty: number } | undefined;
  const qty = leg?.qty ?? 1;
  const direction = leg?.side === 'sell' ? -1 : 1;

  const mid = snapshot?.mid ?? null;
  const pnl = mid !== null ? direction * (mid - fillPrice) * qty * 100 : null;

  return (
    <div className="rounded-lg border border-zinc-700/30 bg-zinc-900/20 p-2 space-y-1.5">
      <div className="flex items-center justify-between">
        <span className="text-[10px] font-bold text-zinc-400 uppercase tracking-wide">Live P&amp;L</span>
        <button
      ...
      {loading && !snapshot ? (
        ...
        <div className="space-y-0.5 text-xs">
          <div className="flex justify-between">
            <span className="text-zinc-400">Current mid</span>
            <span className="text-white font-mono">${mid.toFixed(2)}</span>
          </div>
          <div className="flex justify-between">
            <span className="text-zinc-400">Fill price</span>
            <span className="text-zinc-400 font-mono">${fillPrice.toFixed(2)}</span>
          </div>
          {pnl !== null && (
            <div className="flex justify-between border-t border-zinc-700/30 pt-1">
        ...
        <p className="text-[10px] text-zinc-600 italic">
          {snapshot === null && !loading ? 'Price unavailable — market may be closed' : '—'}
        </p>
      )}
    </div>
  );
                    ...
                    {intent.status === 'executed' && (() => {
                      ...
                      return (
                        <div className="rounded-lg border border-emerald-800/40 bg-emerald-950/20 p-2 space-y-1.5">
                          ...
                          <LivePnL intent={intent} fillPrice={avgFillPrice} />
                        </div>
                      );
                    })()}
